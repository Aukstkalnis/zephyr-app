...
static inline void uart_bluenrg_set_baudrate(struct device *dev, u32_t baud_rate)
{
	...
	UartInstance->UART_BaudRate = baud_rate;
	...
	if (UART->CR_b.OVSFACT == 0) {
		divider = (UART_CLOCK<<7) / (UART_CLOCK_CYCLE16 * UartInstance->UART_BaudRate);
	} else {
		divider = (UART_CLOCK<<7) / (UART_CLOCK_CYCLE8 * UartInstance->UART_BaudRate);
	}
	ibrd = divider >> 7;
	UART->IBRD = ibrd;
...
{
	UART_InitType *UartInstance = UART_STRUCT(dev);
	UartInstance->UART_Parity = parity;
...
static int uart_bluenrg_poll_in(struct device *dev, unsigned char *c)
{
	...
	CKGEN_SOC_Type* clock = CKGEN_SOC;
	SYSTEM_CTRL_Type* _ctrl = SYSTEM_CTRL;
	GPIO_Type *gpio = GPIO;
	UART_Type* uart = UART;
	CKGEN_BLE_Type* ble_clock = CKGEN_BLE;
...
static int uart_bluenrg_fifo_fill(struct device *dev, const u8_t *tx_data,
				  int size)
{
	UART_InitType *uart = UART_STRUCT(dev);
	u8_t num_tx = 0U;
				  ...
				  const int size)
{
	...
	while ((size - num_rx > 0) &&
			UART_GetFlagStatus(UART_FLAG_RXFE)) {
...
{
	return UART_GetITStatus(UART_IT_TX);
}

static int uart_bluenrg_irq_tx_complete(struct device *dev)
{
	return UART_GetFlagStatus(UART_FLAG_TXFE);
}
...
static int uart_bluenrg_irq_is_pending(struct device *dev)
{
	...
	return (
		...
		(UART_GetITStatus(UART_IT_TX) && UART_GetFlagStatus(UART_FLAG_TXFE))
	);
...
static const struct uart_bluenrg_config uart_bluenrg_cfg_##name = {		\
	...
	.pclken = { .bus = DT_UART_BLUENRG_##name##_CLOCK_BUS,	\
		    .enr = DT_UART_BLUENRG_##name##_CLOCK_BITS	\
	},								\
